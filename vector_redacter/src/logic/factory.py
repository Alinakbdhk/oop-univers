...
class ShapeFactory:
    ...
    def create_shape(shape_type: str, start_point: QPointF, end_point: QPointF, color: str = DEFAULT_COLOR) -> Shape:
        """
        Создает фигуру по типу и координатам.

        Args:
            shape_type: Тип фигуры ('rect', 'line', 'ellipse')
        ...
        Raises:
            ValueError: Если неизвестный тип фигуры
        """
        ...
        # Для прямоугольника и эллипса нормализуем координаты
        x = min(x1, x2)
        y = min(y1, y2)
        w = abs(x2 - x1)
        h = abs(y2 - y1)
    ...
    def from_dict(data: Dict[str, Any]) -> Shape:
        ...
        Args:
            data: Словарь с данными фигуры

        Returns:
            Восстановленная фигура или группа

        Raises:
            ValueError: Если неизвестный тип
        """
    ...
    def _create_primitive(data: Dict[str, Any]) -> Shape:
        ...
        # Изменения здесь: передаем параметры в конструкторы
        if shape_type == TYPE_RECT:
            obj = Rectangle(
                props['x'],  # это станет self._x
                props['y'],  # это станет self._y
                props['w'],  # это станет self._w
                props['h'],  # это станет self._h
                color,
                stroke_width
            )
        elif shape_type == TYPE_LINE:
            obj = Line(
                props['x1'],  # это станет self._x1
                props['y1'],  # это станет self._y1
                props['x2'],  # это станет self._x2
                props['y2'],  # это станет self._y2
                color,
                stroke_width
            )
        elif shape_type == TYPE_ELLIPSE:
    ...
    def _create_group(data: Dict[str, Any]) -> Group:
        """Создает группу и рекурсивно восстанавливает детей."""
        group = Group()
        ...
        for child_dict in children_data:
            ...
            # Устанавливаем локальные координаты ребенка
